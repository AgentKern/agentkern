...
mod wasm_registry_tests {
    ...
    fn sample_wasm_module() -> Vec<u8> {
        ...
        wat::parse_str(r#"(module (func (export "evaluate")))"#)
    ...
    async fn test_register_and_list() {
        ...
        let meta = registry.register("test-actor", "1.0.0", &sample_wasm_module(), caps)
    ...
    async fn test_hot_swap() {
        ...
        // Register v1
        registry.register("policy", "1.0.0", &sample_wasm_module(), caps.clone())
        ...
        // Hot-swap to v2
        registry.register("policy", "2.0.0", &sample_wasm_module(), caps)
    ...
    async fn test_capability_routing() {
        ...
        registry.register("actor1", "1.0.0", &sample_wasm_module(), vec![
            Capability { name: "prompt_guard".to_string(), input_schema: None, output_schema: None },
        ]).unwrap();
        
        // Actor 2: carbon_check capability
        registry.register("actor2", "1.0.0", &sample_wasm_module(), vec![
            Capability { name: "carbon_check".to_string(), input_schema: None, output_schema: None },
        ]).unwrap();
    ...
    async fn test_unregister() {
        ...
        registry.register("temp", "1.0.0", &sample_wasm_module(), vec![]).unwrap();
        assert_eq!(registry.list_actors().len(), 1);
    ...
    async fn test_stats() {
        ...
        registry.register("a1", "1.0.0", &sample_wasm_module(), vec![]).unwrap();
        registry.register("a2", "1.0.0", &sample_wasm_module(), vec![]).unwrap();
