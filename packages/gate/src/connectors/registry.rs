...
use super::sdk::{LegacyConnector, ConnectorProtocol, ConnectorHealth, ConnectorResult, ConnectorError};
...
impl ConnectorRegistry {
    ...
    pub fn register(
        ...
        connectors.insert(id, RegisteredConnector {
            connector,
    ...
    pub fn unregister(&self, id: &str) -> ConnectorResult<()> {
        let mut connectors = self.connectors.write();
        connectors.remove(id).ok_or_else(|| {
            ConnectorError::Internal(format!("Connector '{}' not found", id))
        })?;
        Ok(())
    }
    ...
    pub fn enable(&self, id: &str) -> ConnectorResult<()> {
        let mut connectors = self.connectors.write();
        let reg = connectors.get_mut(id).ok_or_else(|| {
            ConnectorError::Internal(format!("Connector '{}' not found", id))
        })?;
        reg.enabled = true;
        Ok(())
...
mod tests {
    ...
    fn test_registry_register() {
        ...
        registry.register("test-1", connector.clone(), vec!["test".into()]).unwrap();
    ...
    fn test_registry_duplicate_error() {
        ...
        registry.register("test-1", connector.clone(), vec![]).unwrap();
    ...
    fn test_registry_stats() {
        let registry = ConnectorRegistry::new();
        registry.register("mock-1", Arc::new(MockConnector::new("mock-1")), vec![]).unwrap();
        registry.register("mock-2", Arc::new(MockConnector::new("mock-2")), vec![]).unwrap();
