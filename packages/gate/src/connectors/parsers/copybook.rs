...
impl CopybookParser {
    ...
    fn parse_field_line(
        ...
        // Find PIC clause
        let (field_type, length, decimals, pic) = if let Some(pic_idx) = parts.iter().position(|&p| p.eq_ignore_ascii_case("PIC") || p.eq_ignore_ascii_case("PICTURE")) {
    ...
    fn parse_pic_clause(pic: &str) -> Result<(CopybookFieldType, usize, usize, String), CopybookParseError> {
    ...
    fn extract_field_value(&self, field: &CopybookField, data: &[u8]) -> Result<String, CopybookParseError> {
...
mod tests {
    ...
    fn test_field_types() {
        ...
        let cust_id = parser.fields().iter().find(|f| f.name == "CUST-ID").unwrap();
        assert_eq!(cust_id.field_type, CopybookFieldType::NumericDisplay);
        assert_eq!(cust_id.length, 10);
        
        let cust_name = parser.fields().iter().find(|f| f.name == "CUST-NAME").unwrap();
    ...
    fn test_group_field() {
        ...
        let group = parser.fields().iter().find(|f| f.name == "CUSTOMER-RECORD").unwrap();
    ...
    fn test_decimal_handling() {
        ...
        let balance = parser.fields().iter().find(|f| f.name == "CUST-BALANCE").unwrap();
