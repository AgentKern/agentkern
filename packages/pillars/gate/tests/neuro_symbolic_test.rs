use agentkern_gate::neural::{IntentClass, NeuralError, NeuralGuard, NeuroSymbolicValidator, RuleAction};
...
fn test_neural_guard_plumbing() -> Result<(), NeuralError> {
    ...
    println!("Safe Action: {:?} (Confidence: {:.2})", safe_result.intent, safe_result.confidence);
    ...
    println!("Malicious Action: {:?} (Confidence: {:.2})", malicious_result.intent, malicious_result.confidence);
    // In mock mode, "delete" -> Malicious/Suspicious
    assert!(matches!(malicious_result.intent, IntentClass::Malicious | IntentClass::Suspicious));
    ...
    println!("Financial Action: {:?} (Confidence: {:.2})", financial_result.intent, financial_result.confidence);
    // In mock mode, "transfer money" -> Financial
    assert!(matches!(financial_result.intent, IntentClass::Financial | IntentClass::Safe));
...
fn test_neuro_symbolic_validator_integration() -> Result<(), NeuralError> {
    ...
    println!("Action: '{}' -> Result: {:?} (Reason: {:?})", action, result.action, result.reason);
    ...
    println!("Large Transfer: '{}' -> Result: {:?}", action_large, result_large.action);
    assert_eq!(result_large.action, RuleAction::Review);
    assert!(result_large.reason.contains("Symbolic rule: review_large_transfer"));

    // 3. Test: "Root Command" (SystemOp + sudo)
    let root_action = "sudo rm -rf /";
    let root_result = validator.validate(root_action)?;
    
    println!("Action: '{}' -> Result: {:?} (Reason: {:?})", root_action, root_result.action, root_result.reason);
