...
async fn test_state_store_concurrency_stress() {
    ...
    for i in 0..10 {
        ...
        handles.push(tokio::spawn(async move {
            for j in 0..100 {
                ...
                updates.insert("last_action".to_string(), serde_json::json!(format!("action-{}", j)));
    ...
    assert!(duration.as_millis() < 500, "State store too slow under contention: {:?}", duration);
}
...
async fn test_non_blocking_alerting_consistency() {
    ...
    for i in 0..10 {
        store.record_step(agent_id, format!("action-{}", i), None).await;
    }
