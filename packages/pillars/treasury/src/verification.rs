...
mod proofs {
    ...
    fn verify_transfer_atomicity() {
        ...
        ledger
            .deposit("agent-a", Amount::from_raw(balance_a))
            .unwrap();
        ledger
            .deposit("agent-b", Amount::from_raw(balance_b))
            .unwrap();
        ...
        rt.block_on(async {
            let request =
                TransferRequest::new("agent-a", "agent-b", Amount::from_raw(transfer_amount));
            ...
            if result.status == crate::transfer::TransferStatus::Completed {
                kani::assert(
                    final_a == balance_a - transfer_amount,
                    "Sender balance mismatch",
                );
                kani::assert(
                    final_b == balance_b + transfer_amount,
                    "Receiver balance mismatch",
                );
            } else {
                kani::assert(
                    final_a == balance_a,
                    "Failed transfer altered sender balance",
                );
                kani::assert(
                    final_b == balance_b,
                    "Failed transfer altered receiver balance",
                );
            }
        });
    }
