...
async fn test_chaos_infused_golden_flow() {
    ...
    let config = ChaosConfig::new()
        .with_protocol(Protocol::AgentKern, 0.10); // 10% failure rate
    ...
    for i in 0..iterations {
        ...
        loop {
            ...
            let result = proxy.maybe_fail(ChaosTarget::Protocol(Protocol::AgentKern)).await;
    ...
    assert!(stats.failures_injected > 0, "Chaos Monitor: No failures injected? Check probability.");
    ...
    assert_eq!(failures, 0, "Resilience Failure: System could not recover from chaos.");
    assert_eq!(successes, iterations, "Resilience Failure: Not all tasks completed.");
}
