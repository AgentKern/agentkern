...
async fn test_lock_persists_across_restart() {
    ...
    // === Phase 2: Verify lock exists with NEW coordinator ===
    {
        ...
        assert!(!conflict_result.granted, "Conflicting lock should be denied");
...
async fn test_queue_persists_across_restart() {
    ...
    // === Phase 2: Verify queue with NEW coordinator ===
    {
        ...
        coord2.release_lock("agent-1", resource).await.expect("Release should succeed");
        ...
        assert_eq!(lock.locked_by, "agent-3", "Higher priority agent should get lock");
    }
...
async fn test_lock_manager_direct() {
    ...
    // Release
    lock_manager.release("agent-1", resource).await.expect("Release should succeed");
