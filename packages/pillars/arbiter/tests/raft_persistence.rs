use agentkern_arbiter::raft_manager::{RaftLockManager, NodeId};
...
async fn test_raft_lock_persistence_across_restarts() {
    ...
    {
        let manager = RaftLockManager::new(node_id, "127.0.0.1:9001".to_string(), path.clone()).await;
        
        // Initialize single-node cluster
        let nodes = std::collections::BTreeMap::from([(node_id, ())]);
        manager.raft.initialize(nodes).await.expect("Failed to initialize raft");

        // Wait for leader election
        tokio::time::sleep(Duration::from_millis(1000)).await;

        // Acquire lock
        let success = manager.acquire_lock(agent_id, resource, 10, 60000).await.expect("Failed to acquire lock");
    ...
    // === Phase 2: Create new manager pointing to same data and verify lock ===
    {
        ...
        for i in 0..40 {
            let metrics = manager.raft.metrics().borrow().clone();
            println!("Attempt {}: State={:?}, Term={}, LastLogIdx={:?}, Applied={:?}", i, metrics.state, metrics.current_term, metrics.last_log_index, metrics.last_applied);
        ...
        let result = manager.acquire_lock("another-agent", resource, 5, 20000).await.expect("Acquire should not error when leader");
        assert!(!result, "Lock should have been DENIED because it exists on disk");
        
        // Let's try to heartbeat it. If it exists, it should return Ok(true).
        let hb_result = manager.heartbeat(agent_id, resource).await.expect("Heartbeat should not error");
