...
const env =
  (import.meta as ImportMeta & { env?: Record<string, string | undefined> })
    .env ?? {};
const AGENTKERN_API_URL =
  env.VITE_AGENTKERN_API_URL?.trim() || "http://localhost:3000";
const AGENTKERN_AUTH_AGENT_ID = env.VITE_AGENTKERN_AUTH_AGENT_ID?.trim();
const AGENTKERN_AUTH_SECRET = env.VITE_AGENTKERN_AUTH_SECRET?.trim();

if (!AGENTKERN_AUTH_AGENT_ID || !AGENTKERN_AUTH_SECRET) {
  console.warn(
    "Playground: AGENTKERN_AUTH_AGENT_ID or AGENTKERN_AUTH_SECRET not set; running in simulated mode or using local defaults.",
  );
}
...
const checkBridgeAvailability = async (): Promise<boolean> =>
  bridgeClient.checkAvailability();
...
const simulateVerify = async (
  ...
  return {
    ...
    reasoning:
      riskScore >= 80 ? "Transaction amount too high" : "All policies passed",
  };
};

const simulateStartIntent = async (
  intent: string,
  steps: number,
): Promise<IntentPath> => {
  await delay(200);
...
const simulatePromptCheck = async (
  prompt: string,
...
const realPromptCheck = async (prompt: string): Promise<PromptCheckResult> => {
  const result = await realVerify(AGENTKERN_AUTH_AGENT_ID, "prompt_scan", {
    prompt,
  });
  ...
  return {
    ...
    threatLevel,
    attackType: result.evaluatedPolicies.find(
      (policyId) => policyId !== "default-deny",
    ),
...
export default function App() {
  ...
  const [promptResult, setPromptResult] = useState<PromptCheckResult | null>(
    null,
  );
  ...
  const [transactions, setTransactions] = useState<
    Array<{ id: string; type: string; amount: number; time: string }>
  >([]);
  ...
  const withBridgeFallback = async <T,>(
    ...
    try {
      ...
      console.warn(
        "Bridge/API request failed, falling back to simulation mode.",
        error,
      );
  ...
  return (
    <div className="app">
        ...
        >
          ...
          >
            <h2 style={{ margin: "0 0 1rem 0" }}>
              👋 Welcome to AgentKern Playground
            </h2>
            <p style={{ opacity: 0.8 }}>
              AgentKern is the{" "}
              <strong>Operating System for the Agentic Economy</strong> -
              infrastructure for enterprises to safely deploy AI agents.
            </p>
            ...
            >
              <p>
                <strong>1. Identity →</strong> Register an agent to get a trust
                score based on behavioral history, TEE attestation, and peer
                endorsements
              </p>
              <p>
                <strong>2. Gate →</strong> Verify that your agent can perform
                actions (transfers, deletes) based on policy rules
              </p>
              <p>
                <strong>3. Synapse →</strong> Start an intent path and watch for
                drift if the agent deviates from expected behavior
              </p>
              <p>
                <strong>4. PromptGuard →</strong> Test prompts for injection
                attacks (try: "ignore previous instructions")
              </p>
            </div>

            <h3 style={{ marginTop: "1.5rem" }}>
              🏛️ The Six Pillars Explained:
            </h3>
            <div
              style={{
                display: "grid",
            ...
            >
              ...
              >
                ...
                <span style={{ opacity: 0.8 }}>
                  Trust scores, reputation, agent lifecycle
                </span>
              </div>
              <div
                style={{
              ...
              >
                <strong>🛡️ Gate</strong>
                <br />
                <span style={{ opacity: 0.8 }}>
                  Policy engine, action verification, TEE
                </span>
              </div>
              <div
                style={{
              ...
              >
                ...
                <br />
                <span style={{ opacity: 0.8 }}>
                  Memory, intent tracking, drift detection
                </span>
              </div>
              <div
                style={{
              ...
              >
                <strong>⚖️ Arbiter</strong>
                <br />
                <span style={{ opacity: 0.8 }}>
                  Resource locks, kill switch, governance
                </span>
              </div>
              <div
                style={{
              ...
              >
                ...
                <br />
                <span style={{ opacity: 0.8 }}>
                  Budgets, micropayments, carbon tracking
                </span>
              </div>
              <div
                style={{
              ...
              >
                ...
                <br />
                <span style={{ opacity: 0.8 }}>
                  Protocol translation (A2A, MCP, ANP)
                </span>
              </div>
            </div>
            ...
            >
              <strong>💡 Bridge Mode:</strong> Playground first tries live Rust
              HTTP APIs, then falls back to simulation if unavailable.
            </div>
      ...
      <header className="header">
        ...
        <nav className="nav">
          <a
            href="https://github.com/AgentKern/agentkern"
            target="_blank"
        ...
        <section className="content">
                    ...
                    >
                      ...
                      <div style={{ fontSize: "0.75rem" }}>
                        ...
                        >
                          <div
                            style={{
                              width: `${agent.reputation.behavioral}%`,
                              height: "100%",
                              background: "#3b82f6",
                              borderRadius: "2px",
                            }}
                        ...
                        >
                          <div
                            style={{
                              width: `${agent.reputation.attestation}%`,
                              height: "100%",
                              background: agent.reputation.attestation
                                ? "#22c55e"
                                : "#dc2626",
                              borderRadius: "2px",
                            }}
                        ...
                        >
                          ...
                          <span>
                            {agent.reputation.networkEndorsements} peers
                          </span>
                        </div>
                        <div
                          style={{
                        ...
                        >
                          <div
                            style={{
                              width: `${agent.reputation.complianceHistory}%`,
                              height: "100%",
                              background: "#8b5cf6",
                              borderRadius: "2px",
                            }}
                        ...
                        >
                          <span>Age Bonus</span>
                          <span>
                            {agent.reputation.ageBonus > 0
                              ? `+${agent.reputation.ageBonus}`
                              : "New Agent"}
                          </span>
                        </div>
                      </div>
          ...
          {activeTab === "gate" && (
            <div className="panel">
              ...
              <p className="description">
                ...
                {bridgeStatus === "connected" && (
                  <strong> (Using Real Policy Engine)</strong>
                )}
              </p>
              ...
              {action === "delete_record" && (
                <div className="form-group">
                  ...
                  >
                    ⚠️ Delete operations are high-risk
                  </p>
                </div>
              )}
              ...
              {!agent && (
                <p className="hint">
                  ⚠️ Register an agent first in the Identity tab.
                </p>
              )}

              {verification && (
                <div
                  className={`result ${verification.allowed ? "success-result" : "error-result"}`}
                >
                  ...
                  <p>
                    ...
                    {verification.evaluatedPolicies.join(", ")}
                  </p>
                </div>
              )}
            </div>
          ...
          {activeTab === "synapse" && (
            <div className="panel">
              <h2>🧠 Synapse</h2>
              <p className="description">
                Track agent intent and detect drift.
              </p>
              ...
              {intentPath && (
                <div className="result">
                  <h4>📍 Intent Path</h4>
                  <p>
                    <strong>Goal:</strong> {intentPath.intent}
                  </p>
                  <div className="progress-section">
                    <label>
                      Progress: {intentPath.currentStep}/
                      {intentPath.expectedSteps}
                    </label>
                    <div className="meter">
                      <div
                        className="meter-fill"
                  ...
                  <div className="drift-section">
                    ...
                    <span
                      className={
                        intentPath.driftScore > 50 ? "error" : "success"
                      }
                    >
          ...
          {activeTab === "arbiter" && (
            <div className="panel">
              <h2>⚖️ Arbiter</h2>
              <p className="description">
                Coordinate access to shared resources and emergency controls.
              </p>
              ...
              {lockResult && (
                <div
                  className={`result ${lockResult.acquired ? "success-result" : "error-result"}`}
                >
                  <h4>
                    {lockResult.acquired ? "🔐 Lock Acquired" : "⏳ Queued"}
                  </h4>
                  {lockResult.acquired ? (
                    <>
                      <p>
                        <strong>Lock ID:</strong> {lockResult.lockId}
                      </p>
                      <p>
                        <strong>Expires in:</strong> {lockResult.expiresIn}s
                      </p>
                      <p>
                    ...
                    <>
                      <p>
                        <strong>Queue Position:</strong> #{lockResult.queue}
                      </p>
                      <p>
                        <strong>Resource:</strong> {resource}
                      </p>
              ...
              <div className="info-box" style={{ marginTop: "1.5rem" }}>
                ...
                <button
                  ...
                  style={{
                    background: killSwitchActive ? "#dc2626" : undefined,
                  }}
                >
                  {killSwitchActive
                    ? "🔴 KILL SWITCH ACTIVE"
          ...
          {activeTab === "treasury" && (
            <div className="panel">
              <h2>💰 Treasury</h2>
              <p className="description">
                Manage agent budgets and micropayments.
              </p>
              ...
              <div className="form-group">
                ...
                <div className="button-group">
                  <button
                    className="secondary"
                    onClick={() => handleAllocateBudget(100)}
                  ...
                  <button
                    className="secondary"
                    onClick={() => handleAllocateBudget(500)}
                  ...
                  <button
                    className="secondary"
                    onClick={() => handleAllocateBudget(1000)}
          ...
          {activeTab === "nexus" && (
            <div className="panel">
              <h2>🔀 Nexus</h2>
              <p className="description">
                Protocol translation gateway for A2A, MCP, and ANP.
              </p>
                    ...
                    {discoveredAgents
                      .filter(
                        (a) =>
                          ...
                          a.protocols.includes(selectedProtocol),
                      )
                      .map((agent) => (
                        <div
                          key={agent.id}
              ...
              <div className="info-box" style={{ marginTop: "1.5rem" }}>
                ...
                <ul>
                  <li>
                    <strong>A2A</strong> - Google's Agent-to-Agent protocol
                  </li>
                  <li>
                    <strong>MCP</strong> - Anthropic's Model Context Protocol
                  </li>
                  <li>
                    <strong>ANP</strong> - Agent Network Protocol (internal)
                  </li>
                </ul>
              </div>
            </div>
          ...
          {activeTab === "promptguard" && (
            <div className="panel">
              ...
              <p className="description">
                ...
                {bridgeStatus === "connected" && (
                  <strong> (Using Real Rust Engine)</strong>
                )}
              </p>
              ...
              <div className="info-box" style={{ marginTop: "1rem" }}>
                ...
                <ul>
                  <li>
                    <code
                      onClick={() =>
                        setPromptText("What is the weather today?")
                      }
                    >
              ...
              {promptResult && (
                <div
                  className={`result ${promptResult.safe ? "success-result" : "error-result"}`}
                >
                  ...
                  {promptResult.attackType && (
                    <p>
                      <strong>Attack Type:</strong> {promptResult.attackType}
                    </p>
                  )}
                  {promptResult.reason && (
                    <p>
                      <strong>Reason:</strong> {promptResult.reason}
                    </p>
                  )}
                </div>
              )}
          ...
          {activeTab === "integrate" && (
            <div className="panel">
              ...
              <p className="description">
                Connect your AI agent to AgentKern in 3 steps.
              </p>
              ...
              >
                ...
                >
                  ...
                  <div style={{ color: "#8b949e", marginTop: "0.5rem" }}>
                    # For Rust
                  </div>
              ...
              >
                ...
                >
                  <div style={{ color: "#8b949e" }}>
                    // TypeScript: HTTP-first setup
                  </div>
                  <div>
                    <span style={{ color: "#ff7b72" }}>const</span> API_URL =
                    process.env.
                    <span style={{ color: "#79c0ff" }}>AGENTKERN_API_URL</span>{" "}
                    ??{" "}
                  ...
                  <div style={{ marginTop: "0.5rem" }}>
                    <span style={{ color: "#ff7b72" }}>const</span> auth ={" "}
                    <span style={{ color: "#ff7b72" }}>await</span> fetch(
                    <span style={{ color: "#a5d6ff" }}>
                  ...
                  <div style={{ paddingLeft: "1rem" }}>
                    headers: {"{"}{" "}
                    <span style={{ color: "#a5d6ff" }}>'content-type'</span>:{" "}
                    <span style={{ color: "#a5d6ff" }}>'application/json'</span>{" "}
                    {"}"},
                  </div>
                  <div style={{ paddingLeft: "1rem" }}>
                    body: JSON.stringify({"{"}
                  ...
                  <div style={{ paddingLeft: "2rem" }}>
                    agent_id: process.env.
                    <span style={{ color: "#79c0ff" }}>
                      AGENTKERN_AUTH_AGENT_ID
                    </span>
                    ,
                  </div>
                  <div style={{ paddingLeft: "2rem" }}>
              ...
              >
                ...
                >
                  ...
                  <div>
                    <span style={{ color: "#ff7b72" }}>let</span> result;
                  </div>
                  <div>
                    <span style={{ color: "#ff7b72" }}>try</span> {"{"}
                  </div>
                  <div style={{ paddingLeft: "1rem" }}>
                    result = <span style={{ color: "#ff7b72" }}>await</span>{" "}
                    fetch(
                    <span style={{ color: "#a5d6ff" }}>
                      {"`${API_URL}/api/v1/gate/verify`"}
                    </span>
                    , {"{"}
                  </div>
                  <div style={{ paddingLeft: "2rem" }}>
                  ...
                  <div style={{ paddingLeft: "2rem" }}>
                    ...
                    <span style={{ color: "#a5d6ff" }}>
                      {'`${auth.token_type ?? "Bearer"} ${auth.token}`'}
                    </span>
                    , <span style={{ color: "#a5d6ff" }}>'content-type'</span>:{" "}
                    <span style={{ color: "#a5d6ff" }}>'application/json'</span>{" "}
                    {"}"},
                  ...
                  <div style={{ paddingLeft: "3rem" }}>
                    ...
                    <span style={{ color: "#a5d6ff" }}>'transfer_funds'</span>,
                    namespace:{" "}
                    <span style={{ color: "#a5d6ff" }}>'default'</span>,
                    context: {"{"} amount:{" "}
                    <span style={{ color: "#79c0ff" }}>5000</span> {"}"}
                  </div>
                  ...
                  <div>
                    {"}"} <span style={{ color: "#ff7b72" }}>catch</span> {"{"}
                  </div>
                  <div style={{ paddingLeft: "1rem", color: "#8b949e" }}>
                    // Safe fallback policy
                  </div>
                  <div style={{ paddingLeft: "1rem" }}>
                    result = {"{"} allowed:{" "}
                    <span style={{ color: "#79c0ff" }}>false</span>, reasoning:{" "}
                    <span style={{ color: "#a5d6ff" }}>
                  ...
                  <div style={{ paddingLeft: "1rem" }}>
                    <span style={{ color: "#ff7b72" }}>throw new</span>{" "}
                  ...
                  <div>
                    ...
                    <span style={{ color: "#d2a8ff" }}>executeTransfer</span>
                    (...);
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginTop: "2rem", display: "flex", gap: "1rem" }}>
                <a
                  href="https://github.com/AgentKern/agentkern/tree/main/docs"
                  target="_blank"
                  rel="noopener noreferrer"
