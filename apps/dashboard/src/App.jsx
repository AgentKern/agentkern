...
function App() {
  ...
  useEffect(() => {
    ...
    const configuredWs =
      env.VITE_AGENTKERN_WS_URL && env.VITE_AGENTKERN_WS_URL.trim();
    const wsUrl =
      configuredWs ||
      `${protocol}//${window.location.hostname}:3000/api/v1/gate/ws/activity`;

    if (!configuredWs) {
      console.warn(
        "Dashboard: VITE_AGENTKERN_WS_URL not set; using default WebSocket URL",
        wsUrl,
      );
    }
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.type === "verification") {
          ...
          setStats((prev) => ({
            ...prev,
            blockedThreats: v.result.allowed
              ? prev.blockedThreats
    ...
    ws.onerror = (err) => {
      console.error("WebSocket error on dashboard connection", err);
    };
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {activity.map((item) => (
                  ...
                  >
                    <td
                      style={{ padding: "1rem 0", color: "var(--text-muted)" }}
                    >
                      {item.time}
                    </td>
