...
use agentkern_synapse::passport::schema::{MemoryPassport, AgentIdentity};
...
async fn test_six_pillars_full_workflow_proof() {
    ...
    if let Some(ref p) = pool {
        ...
        let record = identity_manager.register(
            &agent_id, 
            "GroundTruthBot", 
            "1.0.0", 
            Some("demo")
        ).await.expect("Identity registration failed");
    ...
    let lock_id = arbiter.acquire_lock(resource, &agent_id, 10).await
    ...
    use agentkern_gate::types::{DataRegion as GateRegion, VerificationRequest, VerificationContext};
    ...
    println!("✅ Gate Result: Allowed={}, Reason={}", result.allowed, result.reasoning);
    ...
    match tx_result.status {
        ...
        _ => {
            println!("ℹ️  Treasury Transfer handled (expected error if no balance): {:?}", tx_result.error)
        }
    }
    ...
    let exported_bytes = synapse_exporter.export(&passport, &options)
        .expect("Synapse export failed");
    println!("✅ Synapse Result: Exported {} bytes (Encrypted + Compressed).", exported_bytes.len());
    ...
    nexus.register_agent(card).await.expect("Nexus agent registration failed");
