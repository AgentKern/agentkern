...
export class DnsController {
  ...
  @ApiOperation({
    ...
    description:
      'Returns cached trust resolution. Creates new record if not exists.',
  })
  ...
  @ApiResponse({
    ...
    type: TrustResolutionResponseDto,
  })
  async resolve(
  ...
  @ApiOperation({
    ...
    description:
      'Efficiently resolves multiple trust queries in a single request.',
  })
  @ApiResponse({
    status: 200,
    description: 'Array of trust resolutions',
    type: [TrustResolutionResponseDto],
  })
  async resolveBatch(
  ...
  async reinstateTrust(
    ...
    const record = await this.dnsService.reinstateTrust(
      dto.agentId,
      dto.principalId,
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'Array of trust records',
    type: [TrustRecordResponseDto],
  })
  async getRecordsForPrincipal(
    ...
    const records =
      await this.dnsService.getTrustRecordsForPrincipal(principalId);
