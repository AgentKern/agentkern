...
export class DnsResolutionService {
  ...
  async resolve(query: TrustQuery): Promise<TrustResolution> {
    ...
    if (!record) {
      ...
      record = await this.trustRepository.save(
        this.trustRepository.create(newRecord),
      );

      this.logger.log(
        `Created new trust record for ${query.agentId}:${query.principalId}`,
      );
    }
  ...
  async registerTrust(
    ...
    const saved = await this.trustRepository.save(
      this.trustRepository.create(record),
    );
  ...
  async recordVerificationSuccess(
    ...
    const cacheKey = this.getCacheKey({ agentId, principalId });
    const record = await this.trustRepository.findOne({
   ...
   */
  async getTrustRecordsForPrincipal(
    principalId: string,
  ): Promise<TrustRecord[]> {
