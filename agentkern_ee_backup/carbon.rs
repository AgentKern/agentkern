    ...
    pub fn get_fleet_usage(&self) -> CarbonUsage {
        ...
        let (start, end) = if footprints.is_empty() {
            ...
            let first = footprints
                .first()
                .expect("Non-empty footprints should have first element");
            let last = footprints
                .last()
                .expect("Non-empty footprints should have last element");
...
/// Result of a carbon budget check
#[derive(Debug, Clone)]
pub struct CarbonCheckResult {
    /// Whether the action is allowed based on carbon budget
    pub allowed: bool,
    /// Carbon footprint in grams CO2
    pub footprint_grams: u64,
    /// Message explaining the result
    pub message: String,
}

impl CarbonCheckResult {
    /// Create a new carbon check result
    pub fn new(allowed: bool, footprint_grams: u64, message: String) -> Self {
        Self {
            allowed,
            footprint_grams,
            message,
